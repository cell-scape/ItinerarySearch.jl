...
- **Carrier-scoped schedule sweeps** — `search_schedule` builds every valid itinerary for a carrier (plus optional codeshare partners) across one or more dates; `:direct` universe for fast market discovery, `:connected` for broader coverage via the full connection graph
- **Thread-based parallelism** — per-market search scales across `JULIA_NUM_THREADS` workers with per-thread `RuntimeContext` cache isolation; 4.4× speedup at 4 threads on the demo dataset (see [Parallelism](architecture.md#parallelism))
- **Graph reuse for advanced callers** — `search_schedule(graph, universe)` skips ingest and graph build when the caller has already paid those costs; `build_graph_for_window` builds one graph whose window covers a date range, amortizing build cost across multi-date sweeps
- **Convenience tuple dispatch** — `search_markets(path, (origin, dest, date))` and `search(store, (origin, dest, date))` for one-off queries; vector forms are explicit per-tuple, not cartesian-producted with a separate dates list
...
- **Observability** — structured event log with typed events and JSONL sink, DynaTrace-compatible JSON logging via LoggingExtras TeeLogger, cooperative system metrics polling, Tier 1 instrumentation (rule counters, MCT cascade stats, geographic aggregation); `SpanEvent` emission via optional `event_sinks::Vector{<:Function}` callbacks following the OTLP schema shape
